...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $valor_recebido = array_reduce($input['payment'], function ($sum, $item) {
            return $sum += $this->num_uf($item['amount']);
        }, 0);
        ;
        ...
        $transaction = Transaction::create([
            ...
            'troco'                  => $valor_recebido > 0 ? ($valor_recebido - $final_total) : 0,
            'valor_recebido'         => $valor_recebido,
            //> 0 ? $valor_recebido : $invoice_total['total_before_tax'],
            ...
            if ($il->show_payments == 1) {
                ...
                if (!empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        } elseif ($value['method'] == 'boleto') {
                            $output['payments'][] =
                                [
                                    'method' => $method . ($value['is_return'] == 1 ? ' (' . $il->change_return_label . ')(-)' : ''),
                                    'amount' => $this->num_f($value['amount'], $show_currency, $business_details),
                                    'date'   => $this->format_date($value['vencimento'], false, $business_details)
                                ];
