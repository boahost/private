...
@section('content')
    ...
    <section class="content">
        ...
        @endcomponent
        {!! Form::open([
            'url' => action('\Modules\Repair\Http\Controllers\JobSheetController@saveParts', $job_sheet->id),
            'method' => 'post',
            'id' => 'add_part_form',
        ]) !!}
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('[data-id="quantity"]').on('input', function(e) {
                const row = $(this).closest('tr')
                const quantity = e.currentTarget.value
                const totalAmount = quantity * row.find('[data-id="unit_price"]').val()

                row.find('[data-id="total_amount"]').val((totalAmount || 0).toFixed(2))
            })
        ...
        function job_sheet_parts_row(variation_id) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    $('#search_job_sheet_parts').val('').focus()

                    testes = $(result).on('input', function(e) {
                        const row = $(e.currentTarget).closest('tr')
                        const quantity = row.find('[data-id="quantity"]').val()
                        const totalAmount = quantity * row.find('[data-id="unit_price"]').val()

                        console.log(row, quantity, totalAmount);

                        row.find('[data-id="total_amount"]').val((totalAmount || 0).toFixed(2))
                    })

                    $('table#job_sheet_parts_table tbody').append(testes);
                },
            });
        }
